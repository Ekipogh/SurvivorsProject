...
$zipStagingPath = Join-Path ([System.IO.Path]::GetTempPath()) ("SurvivorsProjectAssetsZip_" + [System.Guid]::NewGuid().ToString("N"))
...
Upload-Artifact -Uri "$env:NEXUS_URL/repository/teamcity-raw/SurvivorsProject/$version/Assets.zip" -Path "Assets.zip"
Upload-Artifact -Uri "$env:NEXUS_URL/repository/teamcity-raw/SurvivorsProject/latest/Assets.zip" -Path "Assets.zip"
